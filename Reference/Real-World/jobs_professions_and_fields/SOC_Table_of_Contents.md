# SOC 2018 Manual — Table of Contents

**Source:** `soc_2018_manual.pdf` (U.S. Bureau of Labor Statistics, 2018 Standard Occupational Classification
Manual) in this same folder. Full structure: 23 major groups → 98 minor groups → 459 broad occupations → 867
detailed occupations.

**Purpose, established 2026-07-26:** the developer wants Toronto Fell Out set up so that a content creator doing
a "Can you beat Toronto Fell Out as [a Welder / an Accountant / a Kindergarten Teacher / etc.]?" challenge run has
*something* to roleplay against, no matter how obscure the chosen real-world job — without going anywhere
near full-depth per-occupation detail (867 entries' worth would be both insane and unrealistic to build game
content around). This file catalogs the structure down to the **minor group** level (98 entries) as the right
grain size: specific enough to mean something, broad enough to stay manageable. See
`SOC_Minor_Group_Summaries.md` (companion file, in progress) for brief 1-2 sentence descriptions of each minor
group — not the full manual's own detailed-occupation definitions.
...
## Not Applicable / Deliberately Excluded

None yet — every major group above is a real, legitimate real-world occupation cluster. Whether every one of
them gets an in-character perk or hook is a separate design question (see the companion summaries file and
the eventual perk-design pass), not a pre-filtering decision made here.
