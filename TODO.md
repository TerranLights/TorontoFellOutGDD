...
- [ ] **Regenerate `Storyline/Minmax-Builds/`'s 35 subfolders with the confirmed MAPLE-BUD codes** — the stat-combination scope is resolved (35 combinations, Luck excluded, full code list in `Storyline/Minmax-Builds/README.md`); the existing 35 folders still use the old MACHINE-coded names (01-MAC through 35-INE) and need deleting/regenerating fresh (empty, no content yet — still blocked on districts/factions/NPCs existing).
